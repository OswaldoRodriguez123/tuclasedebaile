<div id="contenido" style="display: {{visible}};">
<div class="banner">
        <!--<img  class="img-responsive" alt="1440x500" data-src="holder.js/1440x500" src="data:image/png;base64,">-->
<!--     <section class="margin-top-100">
    <div class="container">
    <div class="row"> 
    <div class="margin-top-40 col-md-12">    
        <h2 class="text-center" style=" font-weight: bold;">¡APRENDE A BAILAR SALSA CASINO EN 90 DÍAS!</h2>
    </div>
    </div>
    </div>
    <div class="container margin-top-50">
    <div class="row">
        <div class="col-md-12 text-center">        
                <a href="#/empezar" class="btn btn-azul-claro btn-lg text-uppercase">EMPEZAR</a>        
        </div>
    </div>
    </div>
    </section> -->

    <section class="margin-top-30">
        <div class="container">
        <div class="row"> 
        <div class="col-md-12">    
         
        </div>
        </div>
        </div>
        <div class="container margin-top-10">
        <div class="row">
        <br>
            <div class="col-md-12 text-center" style="padding-top:250px">    
            <h2 class="text-center">Conoce el talentoso equipo que conforma tu clase de baile</h2>
                            
            </div>
        </div>
        </div>
    </section>
</div>

<section class="padding-top-bottom-50">
  <div class="container">
  <div class="row">
    <h3 class="text-center">Nuestros Valores fundamentales</h3>  

    <div class="col-md-4 padding-top-bottom-20 text-center">
        <img src="img/valores_1.png" class="img-circle img-responsive img-center img-150" alt="" >
        <h4>Respeto entre las personas</h4>                    
        <p>El respeto hacia los otros es querernos y respetarnos nosotros mismos, valoramos y nos tomamos en serio este principio en nuestra organización.</p>
    </div>
    <div class="col-md-4 padding-top-bottom-20 text-center">
        <img src="img/valores_2.png" class="img-circle img-responsive img-center img-150" alt="" >
        <h4>Proceso de mejoramiento continuo</h4>  
        <p>La planificación, la ejecución, la supervisión y la restructuración es la guía fundamental que nos impulsa a mejorar nuestros servicios y productos.</p>   
    </div>
    <div class="col-md-4 padding-top-bottom-20 text-center ">
        <img src="img/valores_3.png" class="img-circle img-responsive img-center img-150" alt="" >
        <h4>Servimos a la gente</h4>  
        <p>Cambiamos, triunfamos, evolucionamos, cometemos errores, nos adaptamos, aprendemos. La gente siempre es nuestra prioridad.</p> 
    </div>
  </div>
  </div> 
</section>
<section class="fondo-clouds padding-top-bottom-50">
  <div class="container">
  <div class="row">
    <h2 class="text-center">Nuestra tripulación</h2> 
    <h3 class="text-center">Conoce el talentoso equipo que conforma tu clase de baile</h3> 
  </div>
  </div>
</section>

<section class="fondo-clouds padding-top-bottom-50">
  <div class="container">
      <div class="row padding-20">

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/nas.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_rosado">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Nasly Giraldo ... Profesora de diagnóstico</b><br><br>

                Se encarga de las valoraciones recurrentes y diagnósticos de ingreso de nuestros alumnos, como a su vez, organiza, lidera y promueve las actividades como talleres de formación, acto de grado entre otros.  
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/profile.php?id=100001289688447&fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/nasgiraldo" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/dainer.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Dainer Villalobos ... Directiva – Supervisor</b><br><br>

                Es garante del mantenimiento y cumplimiento tanto de los valores de la organización como de los procesos locales del equipo de trabajo, además coordina, motiva, e incentiva a los colaboradores al encuentro de las metas y objetivos de la academia.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/dainerjose.villalobosvillalobos?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/dainerv17" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/andrea.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_rosado">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Andrea Betancourt ... Productora de eventos</b><br><br>

                Se encarga de la organización desarrollo y logística de nuestros eventos y actividades especiales, tales como, fiestas y talleres además maneja la relación pública con diversas empresas y aliados comerciales. 
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/andrea.betancursanchez?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/andrea.betancur" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="clearfix padding-bottom-20"></div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/robert.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Robert Virona ... Directiva -  CEO</b><br><br>

                Diseña el proyecto, traza las líneas principales de la organización, guía al equipo brindando la preparación, herramientas y entrenamiento para el logro de los objetivos. 
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/vironarobert?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/vironarobert" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/henry.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Henry Fuenmayor – Directiva - Promotor</b><br><br>

                Orienta a nuestros clientes y los asesora brindando la información oportuna, ofrece las diversas líneas de producto y servicios con las que cuenta la organización.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/profile.php?id=100010485076620" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/henryjfuenmayor" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/daiver.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Daiver Villalobos - Profesor de baile </b><br><br>

                Es el instructor de pista, es quien se encarga de ofrecer los estilos de baile que brinda la organización, en especial la salsa casino y la bachata, además contribuye en la organización como instructor personalizado. 
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/DaiverVillalobos?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/daiverv" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="clearfix padding-bottom-20"></div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/grei.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_rosado">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Greicy Belloso – Recepcionista</b><br><br>

                Es quien se encarga de manejar la recepción en la academia, sirve y orienta a nuestros clientes, además es instructora de los niños e instructora de clases personalizada.  
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/greicy.belloso?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/greicybelloso" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/dainer.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Oswaldo Rodríguez – Desarrollador web -  diseños e interfaces</b><br><br>

                Es ingeniero en informática, es quien desarrolla la parte tecnológica, tales como sistemas, páginas web y desarrollo en general, le brinda el toque de avanzada a la organización.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/oswaldorodriguez20?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/oswaldorodriguez123" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/daiver.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Sebastián morcillo – Colaborador recepción</b><br><br>

                Contribuye y se desempeña en la organización como colaborador en diversas áreas y en diferentes departamentos o líneas de productos que ofrecemos en la academia.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/sebastian.marmolejo1" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/sebasmorcillo" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="clearfix padding-bottom-20"></div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/andrea.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_rosado">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Analicia González – Diseño gráfico</b><br><br>

                Proyecta la comunicación visual, es quien se encarga de manejar, crear y desarrollar los diseños gráficos de la academia.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/analicia.gonzalez.319?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/analiciagonzalez" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/heduin.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Heduin Chacín – Community Manager</b><br><br>

                Es el contacto principal con nuestros usuarios en las redes sociales, es quien desarrolla y programa las publicaciones, manteniendo informados a los seguidores de las actividades e información que brinda la academia.
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/heduin.chacin?fref=ts" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="https://www.instagram.com/heduinch" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

        <div class="col-sm-4 div_nosotros">
          <img src="img/nosotros/juan.jpg" class="img-responsive imagen_nosotros">
          <span class="descripcion_nosotros descripcion_nosotros_azul">
              <div class="clearfix padding-top-30"></div>
              <span class="padding-20">
                <b>Juan Carlos chirinos- Producto audio visual</b><br><br>

                Es el desarrollador la producción de contenidos para los medios de comunicación visual o canales que poseemos en nuestra organización. 
              </span>
              <div class="social_nosotros">
                    <a href="https://www.facebook.com/juan.c.chirinos.5" target="_blank"><i class="fa fa-facebook-official"></i></a>
                    <a href="#" target="_blank"><i class="fa fa-instagram"></i></a>
              </div>
          </span>
        </div>

      </div>
  </div>
</section>
<section class="margin-top-40">
   <div class="container">
    <div class="row"> 
        <div class="col-md-12 padding-bottom-50">

            <div class="text-center">
    		  <h3 class="padding-bottom-20">Nuestra historia, misión y visión</h3>
            </div>

            <p>En el año 2015 un grupo de profesionales y especialistas en el gremio del baile y gerencia, hemos decidido unirnos para crear la primera franquicia de alto niveles de estándares en el ecosistema del baile.  Contamos con un equipo de trabajo altamente profesional que reúnen entre todos muchos años de experiencia y calidad artística, con un nivel de conocimiento adecuado para brindar excelencia en nuestras diversas líneas de productos. Rompemos con los protocolos y estándares tradicionales del aprendizaje monótono, para llegar a ofrecer experiencias positivas a todos nuestros alumnos en un ambiente cargado de mucho dinamismo, pero basado en la alegría como principio fundamental. Creemos fielmente que TU CLASE DE BAILE se convertirá en la franquicia del ecosistema del baile con mayor calidad en Latinoamérica.</p>   
    
        </div>
    </div>
   </div>
</section>
<section class="padding-top-30">
    <img  class="img-responsive" alt="1440x500" data-src="" src="img/ofrecemos-6.jpg">
</section>
<section class="padding-top-40 padding-bottom-40 fondo-gray">
   <div class="container">
       <div class="row">
           <div class="col-md-12">
               <h2 class="text-center">"NUESTRO EQUIPO LO HACE POSIBLE"</h2>
           </div>             
       </div>
   </div>
</section>
</div>
<script type="text/javascript-lazy">
   loadjscssfile("js/function/cargar.js", "js");
</script>
